...
\title{\file{aleph-comandos-beta.sty}}
\author{Proyecto Alephsub0\\ Andr\'es Merino\footnote{Escuela de Ciencias Físicas y Matemáticas, Pontificia Universidad Católica del Ecuador} \\ Daniel Lara\footnote{Facultad de Ciencias, Escuela Politénica Nacional}}
\date{2023-12-22\\ Versión beta-2.0.3}
...
\begin{abstract}
    \file{aleph-comandos-beta.sty} es un paquete creado para recopilar varios comandos de uso común entre los colegas de Andrés Merino, dentro de su proyecto personal Alephsub0 (\url{https://www.alephsub0.org/}).
\end{abstract}
...
El paquete \file{aleph-comandos-beta.sty} es parte del conjunto de clases y paquetes creados por Andrés Merino dentro de su proyecto personal Alephsub0. Este paquete está basado en el paquete \file{comandosEPN.sty} del mismo autor y se cambió su nombre para continuar con el mantenimiento del mismo dentro del proyecto Alephsub0.
...
A partir de la versión beta-2.x se incorporan herramientas para la escritura de textos en áreas relacionados a áreas como Topología, Teoría de la Medida y Análisis Funcional. Estas herramientas se irán incorporando en futuras versiones de manera continua durante el 2023 y 2024. Para sugerencias en este aspecto puedes contactarte a la siguiente dirección: \href{mailto:daniel.lara01@epn.edu.ec}{\texttt{daniel.lara01@epn.edu.ec}}

\section{Uso}

Para cargar la clase se utiliza: \cs{usepackage}|{aleph-comandos-beta}|.
...
Para delimitadores, se utilizan las siguientes abreviaciones
\begin{center}
    \begin{tabular}{cc}
    \hline
        Comando & Acción \\
...
\section{Comandos específicos}

En esta sección se encuentran detallados los comandos para áreas específicas

\subsection{Comandos genéricos}

\DescribeMacro{\distancia}
    El comando \cs{distancia} funciona como una abreviación para la notación de la distancia entre dos conjuntos o un punto y un conjunto. Tiene dos argumentos en el formato\\
        \hspace*{3em}\cs{distancia}\marg{conjunto 1}\marg{conjunto 2},\\
    con esto, el comando genera
    \begin{center}
        |\distancia{A}{B}| 
        \hspace{2cm}
...
\subsection{Comandos para Topología}

A continuación se encuentran comandos útiles para la escritura en Topología

\DescribeMacro{\topologia} 
    El comando \cs{topologia} tiene 1 argumento en el formato\\
        \hspace*{3em}\cs{topologia}\marg{conjunto},\\
    con esto, la función genera
    \begin{center}
        |\topologia{X}| 
        \hspace{2cm}
...
\DescribeMacro{\cociente} 
    El comando \cs{cociente} tiene 1 argumento en el formato\\
        \hspace*{3em}\cs{cociente}\marg{conjunto},\\
    con esto, la función genera
    \begin{center}
        |\cociente{X}| 
        \hspace{2cm}
...
\subsection{Comandos para Teoría de Probabilidades}

A continuación se detallas los comandos usados para los distintos tipos de convergencia en Teoría de Probabilidades
...
\subsection{Comandos para Análisis Funcional}

A continuación se detallan los comandos usados para los distintos espacios utilizados en Análisis Funcional. El argumento es opcional y se utiliza para indicar el conjunto sobre el cual se está trabajando. Por defecto el espacio es $\Omega$.
...
\subsection{Comandos para Teoría de la Medida}

\subsubsection{Espacios de medida}

A continuación se detallan los comandos usados para los distintos espacios de medida. Para el caso de los espacios con la sigma álgebra de Borel estos poseen la opción para indicar el conjunto sobre el que están definidos, por defecto este es $X$
...
\subsubsection{Espacios de Lebesgue}

A continuación se detallan los comandos usados para los distintos espacios de Lebesgue. El primer argumento es opcional y se utiliza para indicar el conjunto sobre el cual se está trabajando mientras que el segundo argumento es obligatorio y se utiliza para indicar el exponente de la norma. No obstante, en el caso del espacio $\Lloc$ solo se solicita, de manera opcional, el conjunto sobre el cual se está trabajando.
...
\begin{changelog}[author=Andés Merino,
    sectioncmd=\section]
    % version beta-2.0.3
    \begin{version}[author=Daniel Lara,v=beta-2.0.3,
        date=2023-12-18]
        \added
        \item Abreviación para espacios de funciones localmente integrables
        \changed
        \item Actualización de la documentación y formato de versionamiento
    \end{version}
     % version beta-2.0.2
     \begin{version}[author=Daniel Lara,v=beta-2.0.2,
        date=2023-12-18]
        \added
        \item Abreviaciones para distancias entre conjuntos o punto-conjunto
    \end{version}
    % version beta-2.0.1
    \begin{version}[author=Daniel Lara,v=beta-2.0.1,
        date=2023-12-10]
        \added
        \item Abreviaciones para espacios funcionales
        \item Abreviaciones para espacios métricos y de medida
    \end{version}
    % version b2.0
    \begin{version}[author=Daniel Lara,v=beta-2.0,
        date=2023-02-14]
        \added
        \item Abreviaciones para espacios topológicos y convergencia en Teoría de Probabilidades
    \end{version}
    % version 1.1.1
    \shortversion{v=1.1.,
    date=2021-06-07,
    changes= Se elimina el uso del paquete \texttt{etex}}
    % version 1.1
    \begin{version}[v=1.1,
    date=2020-08-17]
    \fixed
    \item Se incluye un condicional para definir el comando \verb|\C| que tenía problemas con Beamer y XeLaTeX + hyperref
    \added
    \item Comandos con espacios como \verb|\qDimp| y \verb|\qImp|
    \end{version}
    % version 1.0
    \shortversion{v=1.0,
    date=2019-12-17,
    changes=Primera versión del paquete \texttt{aleph-comandos}}
    % version 0.1
    \shortversion{v=0.1,
    date=2018-02-27,
    changes=Primera versión parquete \texttt{comandos-EPN}}
\end{changelog}

\newpage
\DocInput{aleph-comandos-beta.dtx}
